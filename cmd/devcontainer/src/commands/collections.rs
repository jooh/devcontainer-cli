...
use super::common;
use crate::process_runner::{run_process_streaming, ProcessRequest};
...
mod tests {
    ...
    #[test]
    fn features_test_executes_scripts_and_reports_failures() {
        let root = unique_temp_dir();
        let src = root.join("src").join("demo");
        let test = root.join("test").join("demo");
        fs::create_dir_all(&src).expect("feature src");
        ...
        let results = execute_feature_tests(&[root.display().to_string()]).expect("test execution");
